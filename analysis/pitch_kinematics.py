    ...
    while True:
        ...
        for session in sessions:
            ...
            if not headless:
                ...
                print(" │ " + f"Braking Dive Gradient: {PINK}{abs(brake_m):.2f} mm/G{RESET}".ljust(47 + len(PINK) + len(RESET)) + " │")
                print(" │ " + f"Acceleration Squat Gradient: {CYAN}{abs(accel_m):.2f} mm/G{RESET}".ljust(47 + len(CYAN) + len(RESET)) + " │")
  ...
  >> [USE CASE]: DIRECTLY COMPARE CHASSIS PITCH STIFFNESS / ANTI-DIVE BETWEEN TWO SETUP ITERATIONS."""
            ...
            while True:
                ...
                if ans in ['open l1', 'print l1']:
                    ...
                    plt.scatter(brake_g, brake_pitch, alpha=0.3, color='#FF1493', s=4, label='Braking Telemetry')
                    plt.scatter(accel_g, accel_pitch, alpha=0.3, color='#2D8AE2', s=4, label='Accel Telemetry')
                    
                    if len(brake_g) > 10:
                        x_b = np.array([min(brake_g), 0])
                        plt.plot(x_b, brake_m * x_b + brake_c, color='#FF1493', linewidth=3, label=f'Brake Dive: {abs(brake_m):.2f} mm/G')
                    if len(accel_g) > 10:
                        x_a = np.array([0, max(accel_g)])
                        plt.plot(x_a, accel_m * x_a + accel_c, color='#2D8AE2', linewidth=3, label=f'Accel Squat: {abs(accel_m):.2f} mm/G')
                    ...
                    for ax, b_g, b_p, a_g, a_p, bm, bc, am, ac, tit, subt in [
                        (ax1, brake_g, brake_pitch, accel_g, accel_pitch, brake_m, brake_c, accel_m, accel_c, "PRIMARY", f"{file_basename}"),
                        (ax2, r_valid_g[r_valid_g < -0.2], r_valid_pitch[r_valid_g < -0.2], r_valid_g[r_valid_g > 0.2], r_valid_pitch[r_valid_g > 0.2], r_b_m, r_b_c, r_a_m, r_a_c, "REFERENCE", f"{delta_title}")
                    ]:
                        ax.scatter(b_g, b_p, alpha=0.3, color='#FF1493', s=3)
                        ax.scatter(a_g, a_p, alpha=0.3, color='#2D8AE2', s=3)
                        
                        if len(b_g) > 10:
                            xb = np.array([min(b_g), 0])
                            ax.plot(xb, bm*xb+bc, color='#FF1493', lw=3, label=f'Brake Dive: {abs(bm):.2f} mm/G')
                        if len(a_g) > 10:
                            xa = np.array([0, max(a_g)])
                            ax.plot(xa, am*xa+ac, color='#2D8AE2', lw=3, label=f'Accel Squat: {abs(am):.2f} mm/G')
