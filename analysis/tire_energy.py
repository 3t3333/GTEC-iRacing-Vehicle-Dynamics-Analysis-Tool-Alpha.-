            ...
            while True:
                ...
                if ans in ['open l1', 'print l1']:
                    ...
                    if ans == 'open l1':
                        ...
                        file_out = f"F1L1_{timestamp}_{file_basename}.png"
                        if '<' in ans_raw:
                    ...
                    if ans == 'open l3':
                        ...
                        timestamp = datetime.datetime.now().strftime("%Y_%m_%d_%H_%M")
                        file_out = f"F1L3_{timestamp}_{file_basename}.png"
                        if '<' in ans_raw:
                            project_name = ans_raw.split('<')[1].strip().replace('[', '').replace(']', '').strip()
                            from analysis.projects import save_to_project
                            subf = headless_config.get('run_folder') if headless else None
                    ...
                    if ans == 'open l2':
                        ...
                        timestamp = datetime.datetime.now().strftime("%Y_%m_%d_%H_%M")
                        file_out = f"F1L2_{timestamp}_{file_basename}.png"
                        if '<' in ans_raw:
                            project_name = ans_raw.split('<')[1].strip().replace('[', '').replace(']', '').strip()
                            from analysis.projects import save_to_project
                            subf = headless_config.get('run_folder') if headless else None
                    ...
                    timestamp = datetime.datetime.now().strftime("%Y_%m_%d_%H_%M")
                    batch_dir_name = f"F1L4_{timestamp}_{file_basename}"
                    
                    if '<' in ans_raw:
                        project_name = ans_raw.split('<')[1].strip().replace('[', '').replace(']', '').strip()
                        batch_path = os.path.join("projects", project_name, "exports", batch_dir_name)
                    else:
                    ...
                    for s_name, s_mask, s_suffix in sector_configs:
                        ...
                        plt.tight_layout()
                        plt.savefig(os.path.join(batch_path, f"F1L4_{s_suffix}_{file_basename}.png"), dpi=200, bbox_inches='tight')
                        plt.close(fig)
