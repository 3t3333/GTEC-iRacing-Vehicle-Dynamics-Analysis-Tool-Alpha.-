...
def run_aero_mapping(sessions, headless=False, headless_config=None):
    while True:
        ...
        for session in sessions:
            ...
            print("\n ┌" + "─" * 49 + "┐")
            print(" │ " + "[ EMPIRICAL AERO BALANCE ]".ljust(47) + " │")
            print(" │ " + f"Average High-Speed AB: {CYAN}{median_ab:.1f}% Front{RESET}".ljust(47 + len(CYAN) + len(RESET)) + " │")
            print(" │ " + f"Peak Pitch AB (Braking): {PINK}{max_ab:.1f}% Front{RESET}".ljust(47 + len(PINK) + len(RESET)) + " │")
            print(" │ " + f"Minimum AB (Accel):      {min_ab:.1f}% Front".ljust(47) + " │")
            print(" └" + "─" * 49 + "┘")
            ...
            while True:
                ...
                if ans == 'open l1':
                    ...
                    try:
                        ...
                        main_filename = os.path.join(export_dir, f"AeroMap_{file_basename}.png")
                    ...
                    if ans == 'open l2':
                        ...
                        timestamp = datetime.datetime.now().strftime("%Y_%m_%d_%H_%M")
                        file_out = f"L2_Aero_{file_basename}_vs_{os.path.basename(ref_path)}.png"
                        if '<' in ans_raw:
