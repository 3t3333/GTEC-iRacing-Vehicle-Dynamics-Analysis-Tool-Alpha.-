  ...
  >> [USE CASE]: INTERACTIVE PLOTLY HTML EXPORT FOR DYNAMIC RIDE HEIGHT TRACKING.""")
            ...
            while True:
                ...
                if ans == 'open l1':
                    ...
                    # Calculate Aero Balance for the isolated sector
                    # Using the same logic: total_front_load - (static_front * v_g_factor)
                    # For simplicity in the dash, we'll re-calculate the local balance array
                    ...
                    a_bal_f = (a_front_f / total_df_f) * 100.0
                    a_bal_f = np.clip(a_bal_f, 0, 100)
                    ...
                    ax_aero.set_title("Aero Balance (%) & Total Downforce (N)", fontsize=13, pad=15)
                    ax_aero.set_ylabel("Balance (%)", color='#0ea5e9')
                    ax_aero_twin.set_ylabel("Downforce (N)", color='#A020F0')
                    ax_aero.grid(True, linestyle='--', alpha=0.1)
